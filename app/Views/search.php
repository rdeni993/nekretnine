...
<div class="container basic-font">
    <div class="row mt-5">
    <div class="col-md-3 page-navigation">
    ...
    <div class="col-md-9 bg-white in-page basic-font">
        <!-- SEARCH FORM -->
        ...
        <div class="row article-search p-4 bg-light">
                ...
                <select name="property_type" class="form-control" <?php if(!$prop_types){ echo "disabled"; } ?>>
                    <?php if( $prop_types ): ?>
                        <?php foreach( $prop_types as $p_t ): ?>
                            <option value="<?php echo $p_t->ID; ?>" <?php if(@$_GET['property_type'] == $p_t->ID){ echo " selected"; } ?> ><?php echo $p_t->type; ?></option>
                        <?php endforeach; ?>
                    <?php else: ?>
                ...
                <select name="property_rent" class="form-control" <?php if(!$rent_types){ echo "disabled"; } ?>>
                    <?php if( $rent_types ): ?>
                        <?php foreach( $rent_types as $p_t ): ?>
                            <option value="<?php echo $p_t->ID; ?>"<?php if(@$_GET['property_rent'] == $p_t->ID){ echo " selected"; } ?> ><?php echo $p_t->type; ?></option>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <option value="none">(Prazno)</option>
                    <?php endif; ?>
                ...
                <select name="property_location" class="form-control" <?php if(!$locations){ echo "disabled"; } ?>>
                    <?php if( $locations ): ?>
                        <?php foreach( $locations as $p_t ): ?>
                            <option value="<?php echo $p_t->property_location; ?>"<?php if(@$_GET['property_location'] == $p_t->property_location){ echo " selected"; } ?> ><?php echo $p_t->property_location; ?></option>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <option value="none">(Prazno)</option>
                    <?php endif; ?>
            ...
            <div class="col-md-6 my-3 prop-price-form">
                <label>Cijena</label>
                <ul>
                    <li class="text-left"><input class="form-control" type="number" name="prop_price_min" placeholder="npr. Od 0" <?php if(@$_GET['prop_price_min']){ echo "value='" . $_GET['prop_price_min'] . "'"; } /*else{ echo "value='0'"; }*/ ?> /></li>
                    <li class="text-right"><input class="form-control" type="number" name="prop_price_max" placeholder="npr. Do 100"  <?php if(@$_GET['prop_price_max']){ echo "value='" . $_GET['prop_price_max'] . "'"; }/*else{ echo "value='100'"; }*/ ?> /></li>
                </ul>
                <small class="text-secondary">
                    Opseg cijene je između <span class="pp-min">0</span>€ i <span class="pp-max">100</span>€
                </small>
            </div>
            ...
            <div class="col-md-6 my-3 prop-price-form">
                ...
                <ul class="text-right">
                    <li class="text-left"><input class="form-control" type="number" name="prop_size_min" placeholder="npr. Od 0"  <?php if(@$_GET['prop_size_min']){ echo "value='" . $_GET['prop_size_min'] . "'"; }/*else{ echo "value='0'"; }*/ ?> /></li>
                    <li class="text-right"><input class="form-control" type="number" name="prop_size_max" placeholder="npr. Do 100"  <?php if(@$_GET['prop_price_max']){ echo "value='" . $_GET['prop_size_max'] . "'"; }/*else{ echo "value='100'"; }*/ ?> /></li>
                </ul>
                <small class="text-secondary">
                    Opseg veličine je između <span class="ps-min">0</span>m² i <span class="ps-max">100</span>m²
                </small>
            </div>
            ...
            <div class="col-lg-12 custom-search-option">
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        ...
                        <input class="form-control" type="text" <?php if(@$_GET['property_prec_location']){ echo "value='" . $_GET['property_prec_location'] . "'"; } ?> name="property_prec_location" id="property_prec_log" placeholder="Dio grada" />
                    </div>
                </div>
            </div>
        ...
        <div class="row row-eq-height articles mt-5 article-list">
                                ...
                                <div class="card-body text-right">
                                    <div class="row">
                                        <div class="col-lg-12 text-right">           
                                            <a href="<?php echo site_url('article/index/' . $article->property_ID . '/' . urlencode($article->property_title) ); ?>" class="btn btn-success text-white">Pogledaj</a>
                                        </div>
                                    </div>
                                </div>
                            </div> 
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    <?php if($prev_page > 0): ?>
                    <li class="page-item"><a class="page-link" href="<?php echo site_url('search?page=' . $prev_page ); ?>">Prethodna</a></li>
                    <?php endif; ?>
                    <?php if( !empty($articles) && sizeof($articles) >= 50 ): ?>
                    <li class="page-item"><a class="page-link" href="<?php echo site_url('search?page=' . $next_page ); ?>">Sljedeca</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
            </div>
